...
for (let lineIdx = 0; lineIdx < lines.length; lineIdx++) {
    ...
    // Only show lines in our area where depth changes
    if (htmlLine >= 18063 && htmlLine <= 18844 && depth !== prevDepth) {
        const change = depth - prevDepth;
        const arrow = change > 0 ? '+' + change : '' + change;
        console.log(`HTML:${htmlLine} depth:${prevDepth}->${depth} (${arrow}) | ${line.trim().substring(0, 100)}`);
    }
}
